"""Avatar 单人数字人生成 —— 平静播报风格变体。

与 run_demo_avatar_single_audio_to_video.py 的唯一区别在提示词：
1. 默认 negative_prompt 删除了 "static" / "still picture"（负向词会压制安静画面，
   导致模型不停做手势、点头，表演过火），并显式压制夸张表情/动作。
2. negative_prompt 可被输入 JSON 的 "negative_prompt" 字段覆盖。
其余流程（权重加载、INT8、蒸馏、多段续写）与原脚本完全一致。
"""
import os
import json
import time
...
DEFAULT_NEGATIVE_PROMPT = (
    "Close-up, Bright tones, overexposed, blurred details, subtitles, style, works, "
    "paintings, images, overall gray, worst quality, low quality, JPEG compression residue, "
    ...
    "exaggerated facial expressions, dramatic hand gestures, exaggerated head movements"
)
